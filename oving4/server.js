...
// Plasserer denne MIDDLEWARE-funksjonen
...
app.post("/token", (req, res) => {
  jwt.verify(req.headers["x-access-token"], publicKey, (err, decoded) => {
    if (err) {
      ...
      let brukernavnText = decoded.brukernavn;
      console.log("Token ok: brukernavn er: " + brukernavnText);
      let token = jwt.sign({ brukernavn: brukernavnText }, privateKey, {
        expiresIn: 60
      });
      res.json({ jwt: token });
    }
